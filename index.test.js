...
describe('main', () => {
  ...
  test.skip('should successfully call API to book time with earliest date/time', () => new Promise(async done => {
  ...
  test('should beep if found a date between', () => new Promise(async done => {
    process.argv = ['', '', '2024-07-01', '2024-08-31']
    vi.stubEnv('LOCALE', 'en-ca')
    vi.stubEnv('EMAIL', 'test@example.com')
    vi.stubEnv('PASSWORD', '123')
    vi.stubEnv('FACILITY_ID', '555')
    ...
    const playSoundFn = vi.fn()
    player.play = playSoundFn
    ...
    expect(playSoundFn).toHaveBeenCalled()
  }))
})
